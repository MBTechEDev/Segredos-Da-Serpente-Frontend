            ...
            <SheetContent className="w-full sm:max-w-md bg-background border-l border-white/10 flex flex-col p-0 h-full shadow-2xl">
                ...
                <SheetHeader className="p-6 pb-4 border-b border-white/5 bg-card/30 backdrop-blur-md">
                    <SheetTitle className="font-display text-xl flex items-center gap-2">
                        ...
                        <span className="text-muted-foreground text-xs font-body font-normal ml-auto">
                            ({totalItems} {totalItems === 1 ? "místico" : "itens"})
                        </span>
                    </SheetTitle>
                </SheetHeader>
                ...
                {!cart || cart.items?.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="px-6 py-4 bg-emerald/5 border-b border-emerald/10">
                            <div className="flex justify-between text-xs mb-2">
                                <span className="flex items-center gap-1.5 text-emerald-400">
                                    ...
                                    {remainingForFreeShipping > 0 ? `Faltam ${formatMoney(remainingForFreeShipping, currencyCode)}` : "Frete Grátis Liberado!"}
                                </span>
                        ...
                        {/* Itens com a correção de ordenação (getTime) */}
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {[...(cart.items || [])]
                                .sort((a, b) => new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime())
                                .map((item) => (
                                    <div key={item.id} className="flex gap-4 group animate-in fade-in slide-in-from-right-4">
                                        <div className="relative w-20 h-24 bg-white/5 border border-white/10 rounded overflow-hidden">
                                            <img
                        ...
                        <div className="p-6 bg-card/50 border-t border-white/5 space-y-4">
                            ...
                            <Button asChild className="w-full cta-primary shadow-[0_0_20px_rgba(16,185,129,0.2)]">
                                <LocalizedClientLink href="/checkout">Finalizar Pacto</LocalizedClientLink>
                            </Button>
                        </div>
                    </>
                )}
